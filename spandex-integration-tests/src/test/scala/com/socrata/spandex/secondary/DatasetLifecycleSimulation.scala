...
class DatasetLifecycleSimulation extends FunSuiteLike
  ...
  val secondary = new TestSpandexSecondary(client.client, indexName, client.client.dataCopyBatchSize, 64)
  ...
  test("Entire lifecycle of a dataset") {
    ...
    // We should see 10 new column_value docs in Elasticsearch
    ...
    // The column mapping and any related column values should be removed from the index
